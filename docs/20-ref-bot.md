...
**WhatsApp:** bot hattı **`905325555226`** (`354fb14`, 19 Ağu). Tarihçe: `905325555226` → `905322080888` (`0f77218`) → geri `905325555226` (`354fb14`). Son dönüş **yalnız bot hattını** taşıdı; yasal sayfa telefonu ayrı kaldı (aşağıdaki iki-yüzey paragrafı, KARAR 518). Bot Meta tarafında bu hatta bağlı — **KARAR 396 kapandı**, numara yayını artık B19'a kilitli değil.

**İki yüzey kuralı (KARAR 518).** Bu sitede WhatsApp numarası **tek** değil, **iki ayrı yüzeydir** ve eşitlenmezler:
1. **Bot hattı** — `src/lib/api.ts → WHATSAPP_URL` tek sabiti (KARAR 213). Yüzen buton + `/iletisim` Kanallar kartı + Footer linki üçü bunu import eder. Bugünkü değer `905325555226`.
2. **Yasal telefon** — beş yasal/iletişim sayfasındaki düz metin `+90 532 208 0888` (`iletisim` · `gizlilik` · `hakkimizda` · `teslimat-iade` · `mesafeli-satis`). Sabitten okumaz, sayfada yaşar.

İkisinin farklı olması tutarsızlık **değil**, tasarımdır. Bundan sonra numara değişim talimatı **hangi yüzeyi hedeflediğini adlandırmak zorundadır**; "site numarası" ifadesi tek başına belirsizdir ve uygulanmaz.
...
- **WhatsApp tek-kaynak:** `src/lib/api.ts → WHATSAPP_URL = 'https://wa.me/905325555226'` (bot hattı — Meta TEST hattı borcu 19 Ağu'da kapandı). Floating buton + Kanallar + Footer üçü import eder, gömme yasak. wa.me uluslararası format kuralı (ülke kodu + baştaki 0 düşer). Yasal sayfa telefonu bu sabitten okumaz — KARAR 518.
