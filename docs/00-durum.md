...
| Deploy hook | ⚠ **Ölçüldü (11 Eyl, Claude.ai Vercel MCP):** `notion-content-update` (`tZR9LcwJq9`) **günlük koşuyor**, ~24 saat arayla, son koşum 11 Eylül — ama `ref: astro-iskelet` · `target: null` (**preview**, production değil) ve üretilen commit **sekiz ardışık koşumda `7bf265b`'de hiç değişmedi**. Yani gece tazeleme **çalışıyor, ölü dalı preview olarak yeniden inşa ediyor, production'a hiç değmiyor** — her içerik değişikliği elle `main` push'u bekliyor. Yenisi `notion-content-update-main` → `main` **oluşturuldu ama çağıran yok** — **B64** (teşhis netleşti, çözüm ayrı tur). Bu satır **yazılandır**: kaynağı CC'ye kapalı yüzey, üretilemez |
